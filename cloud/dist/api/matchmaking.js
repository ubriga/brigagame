...
function blockedReason(u) {
    ...
    if (u.banned_until && String(u.banned_until) > nowIso())
        return `החשבון חסום עד ${u.banned_until}.`;
    return null;
}
...
async function claimWaitingMatch(env, matchId, userId) {
    const r = await env.DB.prepare("UPDATE matches SET p2 = ?, status = 'active' WHERE id = ?"
...
async function sweepExpiredOffers(env) {
    ...
    const iso = nowIso();
    await env.DB.prepare("INSERT OR IGNORE INTO match_offer_declines (match_id, user_id, created_at)"
    ...
    await env.DB.prepare("DELETE FROM match_offer_declines WHERE created_at < datetime('now', '-1 day')").run();
}
let sweepLastRun = 0;
/** Port of app.py sweep_stale_matches (before_request housekeeping). */
export async function sweepStaleMatches(env) {
    const now = Date.now() / 1000;
    if (now - sweepLastRun < STALE_SWEEP_INTERVAL_SECONDS)
        return;
    sweepLastRun = now;
    ...
    let aborted = 0;
    const stale = await env.DB.prepare("SELECT id FROM matches WHERE status = 'active' AND updated_at < ?").bind(activeCutoff).all();
    for (const row of stale.results) {
        const cur = await env.DB.prepare("UPDATE matches SET status = 'aborted', winner = NULL,"
    ...
    const purgedCur = await env.DB.prepare("DELETE FROM matches WHERE status = 'waiting' AND updated_at < ?").bind(waitingCutoff).run();
...
export async function offerToPresentPlayer(env, matchId, ownerId) {
    const now = Date.now() / 1000;
    await sweepExpiredOffers(env);
    const live = await env.DB.prepare("SELECT 1 FROM match_offers WHERE match_id = ? AND expires_at > ?")
    ...
    const row = await env.DB.prepare("SELECT u.id FROM users u WHERE u.id != ?"
    ...
    if (!row)
        return false;
    try {
        await env.DB.prepare("INSERT INTO match_offers (match_id, invited_user_id, expires_at, created_at)"
            + " VALUES (?,?,?,?)")
...
async function activateMatch(env, matchId) {
...
export async function handleMatchmaking(env, request, path) {
    ...
    if (path === "/api/matches/quick" && method === "POST") {
        ...
        const now = Date.now() / 1000;
        await sweepExpiredOffers(env);
        const existing = await env.DB.prepare("SELECT match_id, expires_at FROM match_offers WHERE invited_user_id = ? AND expires_at > ?"
        ...
        const waiting = await env.DB.prepare("SELECT m.id FROM matches m WHERE m.mode = 'quick'"
        ...
        const iso = nowIso();
        await env.DB.prepare("INSERT INTO matches (id, code, mode, status, p1, state, version, created_at, updated_at)"
            + " VALUES (?,?,?,?,?,?,0,?,?)")
    ...
    if (path === "/api/matches/join" && method === "POST") {
        ...
        const m = await env.DB.prepare("SELECT id, p1 FROM matches WHERE code = ?").bind(code).first();
        if (!m)
            return json({ error: "not_found", error_he: "קוד לא נמצא." }, 404);
        const uid = Number(u.id);
        if (Number(m.p1) === uid) {
    ...
    if (offerAction && method === "POST") {
        ...
        const now = Date.now() / 1000;
        if (action === "decline") {
            const cur = await env.DB.prepare("DELETE FROM match_offers WHERE match_id = ? AND invited_user_id = ?").bind(mid, uid).run();
            const declined = Number(cur.meta?.changes ?? 0) === 1;
            if (declined) {
                await env.DB.prepare("INSERT OR IGNORE INTO match_offer_declines (match_id, user_id, created_at) VALUES (?,?,?)")
                    .bind(mid, uid, nowIso()).run();
            }
            return json({ ok: true, declined });
        }
        // accept
        const offer = await env.DB.prepare("SELECT * FROM match_offers WHERE match_id = ? AND invited_user_id = ? AND expires_at > ?")
            .bind(mid, uid, now).first();
        if (!offer) {
            await env.DB.prepare("DELETE FROM match_offers WHERE match_id = ? AND invited_user_id = ?").bind(mid, uid).run();
