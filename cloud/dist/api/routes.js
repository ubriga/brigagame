...
export async function handleApi(env, request, path) {
    ...
    if (invitePeek && method === "GET") {
        ...
        if (rl_inv)
            return rl_inv;
        const ctl = (await getControls(env)).invite_system ?? {};
        if (ctl.enabled !== true)
            return json({ error: "invite_disabled", valid: false }, 403);
        const row = await env.DB.prepare("SELECT i.id, i.claimed_by, u.name AS inviter_name FROM invites i"
    ...
    if (path === "/api/invites" && method === "POST") {
        ...
        await env.DB.prepare("INSERT INTO invites (code, inviter_id, created_at) VALUES (?,?,?)")
    ...
    if (path === "/api/invites/claim" && method === "POST") {
        ...
        if (!inv)
            return json({ error: "invite_invalid", error_he: "קישור ההזמנה לא תקף." }, 404);
        ...
        const used = await env.DB.prepare("SELECT COUNT(*) AS c FROM invites WHERE claimed_by = ?").bind(Number(u.id)).first();
        if (Number(used?.c ?? 0) > 0)
            return json({ error: "invite_already_used",
                error_he: "כבר נכנסת בעבר דרך הזמנת חבר." }, 409);
        const upd = await env.DB.prepare("UPDATE invites SET claimed_by = ?, claimed_at = ? WHERE code = ? AND claimed_by IS NULL")
            .bind(Number(u.id), nowIso(), code).run();
        if (Number(upd.meta?.changes ?? 0) !== 1)
            return json({ error: "invite_claimed", error_he: "ההזמנה הזו כבר נוצלה." }, 409);
        const tagName = String(ctl.tag_name ?? "מגייס").slice(0, 40);
        await env.DB.prepare("INSERT OR IGNORE INTO user_tags (user_id, tag, granted_at, source) VALUES (?,?,?,?)")
            .bind(Number(inv.inviter_id), tagName, nowIso(), "invite").run();
        await env.DB.prepare("INSERT INTO messages (user_id, title, body, created_at) VALUES (?,?,?,?)")
            .bind(Number(inv.inviter_id), `תג חדש: ${tagName}! 🎖️`, `${String(u.name ?? "שחקן")} נכנס דרך ההזמנה שלך - קיבלת את התג '${tagName}'. אפשר לראות אותו בעמוד התגים.`, nowIso()).run();
        await audit(env, request, Number(u.id), "invite.claim", "invite", code, { inviter_id: Number(inv.inviter_id), tag: tagName });
