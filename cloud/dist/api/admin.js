...
export async function handleAdminApi(env, request, path) {
    ...
    if (path === "/api/admin/users" && method === "GET") {
        ...
        const rows = await db.prepare("SELECT * FROM users WHERE email LIKE ? OR name LIKE ? ORDER BY id DESC LIMIT 100")
            .bind(term, term).all();
    ...
    if (modMatch && method === "POST") {
        ...
        if (action === "ban") {
            ...
            if (!Number.isFinite(hours))
                return json({ error: "bad_hours" }, 400);
    ...
    if (coinsMatch && method === "POST") {
        ...
        const after = await db.prepare("SELECT coins FROM users WHERE id = ?").bind(uid).first();
    ...
    if (path === "/api/admin/maintenance" && method === "POST") {
        ...
        await db.prepare("INSERT INTO settings (key, value) VALUES ('maintenance', ?)"
            + " ON CONFLICT(key) DO UPDATE SET value = excluded.value")
    ...
    if (path === "/api/admin/gameplay-controls" && method === "POST") {
        ...
        if (body.reset === true) {
            ...
            await db.prepare("INSERT INTO settings (key, value) VALUES ('gameplay_controls', ?)"
                + " ON CONFLICT(key) DO UPDATE SET value=excluded.value")
    ...
    if (cosmMatch && method === "POST") {
        ...
        await db.prepare("INSERT INTO cosmetic_overrides (item_id, price, available, updated_by, updated_at)"
    ...
    if (path === "/api/admin/coupons" && method === "POST") {
        ...
        else if (kind === "item" && CATALOG[body.item_id]) {
            amount = 0;
        ...
        if (existing)
            return json({ error: "code_exists" }, 400);
        await db.prepare("INSERT INTO coupons (code, kind, amount, item_id, max_uses, expires_at, created_by, created_at)"
            + " VALUES (?,?,?,?,?,?,?,?)")
    ...
    if (path === "/api/admin/audit" && method === "GET") {
        const rows = await db.prepare("SELECT a.id, a.created_at, a.action, a.target_type, a.target_id,"
    ...
    if (path === "/api/admin/matches" && method === "GET") {
        const rows = await db.prepare("SELECT id, mode, status, version, created_at, updated_at FROM matches"
