...
export function botChooseWeapon(state, rng = defaultRng) {
    const controls = state.bot_controls ?? {};
    const profile = state.ai_profile ?? {};
    const ammo = (state.bot_ammo ??= {});
    if (controls.enabled === false || controls.special_weapons === false)
        return "standard";
    const available = ["double_bomb", "homing_missile", "cluster_shell"].filter((w) => (controls[w] ?? true) && (controls.infinite_ammo || (ammo[w] ?? 0) > 0));
    if (!available.length)
        return "standard";
...
export function applyBotTactics(m, rng = defaultRng) {
    const state = m.state;
    const controls = state.bot_controls ?? {};
    const profile = state.ai_profile ?? {};
    const events = [];
 ...
 */
export function executeShot(m, side, angle, power, weapon = "standard", mega = false, userId = null, spendAmmo = null, rng = defaultRng, now) {
    ...
    if (weapon !== "standard") {
        if (side === "p2" && m.p2_ai) {
            ...
            if (!controls.infinite_ammo) {
                if ((ammo[weapon] ?? 0) < 1)
                    return [null, { status: 400, body: { error: "no_ammo" } }];
                ammo[weapon] -= 1;
            }
        }
        else {
    ...
    if (mega) {
        ...
        if (charges < 1)
            return [null, { status: 400, body: { error: "no_ability", error_he: "יכולת המגה כבר נוצלה." } }];
    ...
    if (side === "p2" && m.p2_ai) {
        ...
        while (history.length > Math.max(1, depth))
            history.shift();
