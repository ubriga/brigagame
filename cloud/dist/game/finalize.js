...
export async function addCoins(db, userId, delta, reason, ref = "") {
    if (reason === "match_win")
        delta = Math.max(0, Math.min(MAX_COINS_PER_WIN, Math.trunc(delta)));
    await db.run("UPDATE users SET coins = coins + ? WHERE id = ?", [delta, userId]);
    await db.run("INSERT INTO transactions (user_id, delta, reason, ref, created_at) VALUES (?,?,?,?,?)", [userId, delta, reason, ref, nowIso()]);
}
export function rankLossPoints(currentPoints, versusAi = false, aiRankLevel) {
    ...
    if (!versusAi)
        return Math.min(currentPoints, humanLoss);
...
export async function finalizeMatch(db, m, winnerSide, xp) {
    ...
    for (const [side, outcome] of [[winnerSide, "win"], [loserSide, "loss"]]) {
        ...
        if (outcome === "win")
            total = Math.min(MAX_COINS_PER_WIN, total);
        ...
        if (outcome === "win") {
            ...
            await db.run("UPDATE users SET rating = MAX(0, rating - ?), rank_points = MAX(0, rank_points + ? - ?), losses = losses + ?, matches_played = matches_played + 1 WHERE id = ?", [delta, damageXp, lossPts, practice ? 0 : 1, uid]);
...
export async function resolveTimeLimit(db, m, xp, now) {
    ...
    if (t >= suddenAt && !m.state.sudden_death) {
        ...
        if (t < started + MATCH_DURATION_SECONDS)
            return [event];
    }
    if (t < started + MATCH_DURATION_SECONDS)
        return [];
