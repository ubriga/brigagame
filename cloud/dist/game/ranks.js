 ...
 */
const L = (level, key, name_he, abbr_he, group, req) => ({ level, key, name_he, abbr_he, group, wins_required: req, insignia: `assets/ranks/rank-${String(level).padStart(2, "0")}-${key}.svg` });
...
export function rankPayload(points) {
    points = Math.round(points * 10) / 10;
    const level = levelForWins(points);
    const cur = rankForLevel(level);
    const out = { ...cur, xp: points, wins: points, tower_level: level, upgrade_level: level + 1, upgrade_levels_total: 19 };
    if (level < MAX_LEVEL) {
...
export function rankUpInfo(winsBefore, winsAfter) {
