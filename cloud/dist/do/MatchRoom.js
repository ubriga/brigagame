...
export class MatchRoom {
    ...
    async execAction(sess, msg) {
        ...
        if (msg.type === "fire") {
            ...
            if (weapon !== "standard" && !(sess.side === "p2" && m.p2_ai) && this.env.DB) {
                const res = await this.env.DB.prepare("UPDATE user_items SET qty = qty - 1 WHERE user_id = ? AND item_id = ? AND qty > 0")
            ...
            const [result, err] = executeShot(m, sess.side, Number(msg.angle), Number(msg.power), weapon, Boolean(msg.mega), sess.userId, debited
                ? () => { if (callbackConsumed)
                    return false; callbackConsumed = true; return true; }
                : (uid, w) => this.spendAmmoSync(uid, w), defaultRng);
            if (err) {
                if (debited && !callbackConsumed && this.env.DB) {
                    await this.env.DB.prepare("UPDATE user_items SET qty = qty + 1 WHERE user_id = ? AND item_id = ?")
                        .bind(sess.userId, weapon).run();
                }
        ...
        if (msg.type === "move") {
            ...
            if (left < 1)
                return { events: null, err: { status: 400, body: { error: "no_move", error_he: "ההזזה כבר נוצלה." } } };
    ...
    async webSocketMessage(ws, message) {
        ...
        if (!sess || !this.match || this.match.status !== "active")
            return;
        const m = this.match;
        try {
            switch (msg.type) {
    ...
    async alarm() {
        ...
        let [angle, power] = aiChooseShot(m.state, "p2", m.mode ?? "ranked", Number(m.state.ai_rank_level ?? 1), profile, defaultRng);
        ...
        const [result, err] = executeShot(m, "p2", angle, power, weapon, this.botWantsMega(), null, null, defaultRng);
        if (!err && result) {
            events.push(...result.events);
            m.version += 1;
            if (result.won) {
                await this.finalize("p2");
                events.push({ type: "match_end", winner_side: "p2", reason: "tower_destroyed" });
            }
            await this.persist();
            await this.recordEvents(events);
            this.broadcast({ type: "events", version: m.version, events });
    ...
    botWantsMega() {
        ...
        const controls = m.state.bot_controls ?? {};
        const profile = m.state.ai_profile ?? {};
        if (controls.tactical_mega === false)
            return false;
    ...
    async persist() {
        ...
        if (this.env.DB && this.match) {
            const m = this.match;
            await this.env.DB.prepare("UPDATE matches SET state=?, version=?, status=?, winner=?, updated_at=? WHERE id=?")
                .bind(JSON.stringify(m.state), m.version, m.status, m.winner ?? null, new Date().toISOString(), m.id).run();
        }
    }
    broadcast(payload) {
