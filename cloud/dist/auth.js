...
async function googleKeys() {
    if (jwksCache && Date.now() - jwksCache.fetchedAt < 6 * 3600_000)
...
export async function verifyGoogleCredential(credential, clientId) {
    if (!clientId)
        throw new Error("server_missing_client_id");
...
export async function getOrCreateUser(db, email, name, picture) {
    const now = new Date().toISOString();
    let row = await db.get("SELECT * FROM users WHERE email = ?", [email]);
    if (!row) {
        await db.run("INSERT INTO users (email, name, picture, coins, rating, created_at, last_login) VALUES (?,?,?,?,?,?,?)", [email, name, picture, STARTING_COINS, START_RATING, now, now]);
        const created = await db.get("SELECT id FROM users WHERE email = ?", [email]);
        await db.run("INSERT INTO transactions (user_id, delta, reason, created_at) VALUES (?,?,?,?)", [created.id, STARTING_COINS, "welcome_grant", now]);
...
export async function createSession(db, userId) {
    ...
    await db.run("DELETE FROM sessions WHERE user_id = ? AND id NOT IN"
        + " (SELECT id FROM sessions WHERE user_id = ? ORDER BY created_at DESC LIMIT ?)", [userId, userId, MAX_SESSIONS_PER_USER - 1]);
    await db.run("INSERT INTO sessions (token_hash, user_id, created_at, expires_at) VALUES (?,?,?,?)", [hash, userId, now.toISOString(), expires.toISOString()]);
    return token;
}
