...
export const BOT_TIER_WIN_BONUS: Record<string, number> = { medium: 0, hard: 3, ultra: 6 };
...
export function winRewardCoins(damageDealt: number, towerMaxHp: number, opponentRankLevel = 1, botTier?: string): number {
...
export function rankFor(rating: number): string {
  let name = RANKS[0][1];
  for (const [threshold, rname] of RANKS) {
    if (rating >= threshold) name = rname; else break;
  }
  return name;
}
...
export interface Weapon { damage: number; radius: number; cooldown: number; }
